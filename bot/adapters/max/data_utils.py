...
def format_progress_attempts(attempts: list) -> str:
    ...
    for attempt in attempts:
        ...
        elif attempt["course_name"] == "Обучение по продукту":
            block_lines = [
                f"🗓️ Дата прохождения: {formatted_date}",
        ...
        elif attempt["course_name"] == "Регулярный менеджмент":
            block_lines = [
                f"🗓️ Дата прохождения: {formatted_date}",
...
def get_max_accuracy_item(data):
    ...
    """
    # Если передан одиночный кортеж, превращаем его в список из одного элемента
