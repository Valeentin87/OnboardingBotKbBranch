...
def main_menu_keyboard(educ_button_name:str = "Обучение по продажам", status_user:str = "new_employer"):
    ...
    if status_user == "new_employer" and educ_button_name != "Регулярный менеджмент":
        kb.row(CallbackButton(text="🏢 О компании", payload="about_company"))
    if educ_button_name != 'Регулярный менеджмент':
    ...
    kb.row(CallbackButton(text="🎓 Дополнительное образование", payload="additional_education"))
    if educ_button_name != 'Регулярный менеджмент':
        kb.row(
...
def regular_managment_kb():

    kb = KeyboardBuilder()
    kb.row(CallbackButton(text="📚 Регулярный менеджмент", payload="regular_managment"))
...
async def variants_questions_kb(question_data: dict, user_id: int) -> list[list[dict]]:
    ...
    for i in range(1, 5):
        ...
        payload=f'user_answer::{current_variant + "_correct" if  current_variant == correct_answer else current_variant}'
        await save_cursor(user_id, extra_data = dict(call_answer = payload))
        kb.add(CallbackButton(text=current_variant, payload = payload))
