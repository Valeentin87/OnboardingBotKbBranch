...
class GamificationService:
    ...
    def get_result_by_lesson(self, user_id: int, course_name: str, lesson_name: str):
        """Возвращает результат прохождения по уроку для конкретного пользователя
        и конкретного курса обучения, переданных в значениях аргументов функции"""
        try:
            data = self._load_data()
            user_key = str(user_id)
            user_data = data.get(user_key, {})
            
            lesson_results = user_data.get('lesson_results', {})
            course_data = lesson_results.get(course_name, {})
            lesson_data = course_data.get(lesson_name, {})
            logger.info(f'{lesson_data=}')
            return lesson_data     
        except Exception as e:
    ...
    def record_course_completion_attempt(
        ...
        if course_name in ['Другой сотрудник', "Обучение по продукту"]:
            lessons_completed = 7
    ...
    def get_lessons_completed(
        ...
        # if course_name == 'Обучение по продукту':
        #     course_name = 'Другой сотрудник'
    ...
    def get_all_courses_name(self):
        ...
        try:        
            all_courses_name.remove('Другой сотрудник')
        except ValueError:
            logger.info('Другой сотрудник')
    ...
    def get_full_completed_lessons(self, course_name: str, user_id: int) -> tuple[float|int, float|int]:
        ...
        try:
            ...
            if user_data:
                ...
                if courses_data and course_name in courses_data:
    ...
    def reset_user_course_progress(self, user_id: int, course_name: str = "Обучение по продажам") -> Dict[str, Any]:
        ...
        if course_name == 'Обучение по продукту':
            data[user_key]['lesson_results']['Обучение по продукту'] = {}
